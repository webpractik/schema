...
export class CheckUniqueTag extends AbstractCheckSchema {
    ...
    protected errDescription = 'Присутствует уникальное значение поля "tag" в рамках одного файла';

    public isValid(schema: string): boolean {
        const selectionsWithTags = this.schema.selections.filter(selection => selection.tag);
